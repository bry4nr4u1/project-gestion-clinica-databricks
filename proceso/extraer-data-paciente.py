...
dbutils.widgets.text("raw_file", "paciente.csv")
...
dbutils.widgets.text("bronze_table", "paciente")
...
# Definir el esquema del CSV pacientes
pacientes_schema = StructType([
    StructField("identificacion", IntegerType(), False),
    ...
    StructField("tipo_sangre", StringType(), True)
])
...
df_raw = spark.read.option("header", "true").schema(pacientes_schema).csv(csv_path)
