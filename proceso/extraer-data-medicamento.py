...
# MAGIC %md
# MAGIC # Extraer datos medicamentos
...
dbutils.widgets.text("raw_file", "medicamento.csv")
...
dbutils.widgets.text("bronze_table", "medicamento")
...
# Definir el esquema del CSV medicamentos_catalogo
medicamentos_schema = StructType([
    StructField("codigo_medicamento", StringType(), False),
    StructField("nombre_medicamento", StringType(), True),
    StructField("principio_activo", StringType(), True),
    ...
    StructField("precio_unitario", IntegerType(), True),
...
df_raw = spark.read.option("header", "true").schema(medicamentos_schema).csv(csv_path)
